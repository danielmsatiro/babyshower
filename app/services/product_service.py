...
def serialize_product(product: ProductModel) -> dict:
    ...
    url = {"questions": url_for('bp_api.bp_questions.get_product_questions', product_id=product_serialized['id'])}
    product_serialized.update(url)

    for i in range(len(product_serialized["categories"])):
        product_serialized["categories"][i] = product_serialized["categories"][i]["name"]
