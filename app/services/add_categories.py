...
def add_categories_if_empty():
    """If table categories is empty this function add:
    ["0 a 3 meses", "4 a 6 meses", "7 a 9 meses", "10 meses a 1 ano", "2 anos",
    "3 a 5 anos", "carrinhos", "roupas", "quarto do bebê", "cadeiras", "maternidade",
    "banho de bebê", "brinquedos", "lactação", "segurança para bebê", "saúde para bebê"]
    """

    session: Session = db.session
    categories = []

    if not session.query(CategoryModel).all():
        ...
        session.commit()
        return "Sucess: Categories added"
    return "Error: Categories are already in database"
